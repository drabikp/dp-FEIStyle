\section{Prehľad problematiky v oblasti MR}
...
\section{Teoretický základ pre tvorbu MR aplikácie}
...
\section{Biologická podstata prenosu nervového vzruchu}\label{nervova-sustava}
...
\subsubsection{Animácia skoku nervového vzruchu}
...
Ďalej sme do Blueprintu vložili po deväť\footnote{Náš model obsahuje deväť Ranvierovych zárezov.} inštancií \texttt{BP\_Morphing}, \texttt{BP\_SplineMovement} a \texttt{BP\_Impulse}, a deväť nových objektov typu 
...
Medzi premenné Blueprintu sme pridali novú premennú typu \texttt{float} s názvom \texttt{AnimationSpeed}, ktorá bude slúžiť na zmenu rýchlosti celej animácie. Potom sme vytvorili novú funkciu s názvom \texttt{InitMorphingAndSplineMovement}, ktorá má tri parametre typu \texttt{Child Actor Component Object Reference} pre inštancie \texttt{BP\_SplineMovement}, \texttt{BP\_Morphing} a \texttt{BP\_Impulse}, a jeden parameter typu 
...
%\subsubsection{Charakter výskumnej vzorky}
Výskumnú vzorku tvorilo šestnásť študentov septimy a oktávy Školy pre mimoriadne nadané deti a gymnázia v Bratislave. Z pohľadu nacionálií vzorku tvorilo osem mužov a osem žien, všetci pochádzali z Bratislavského samosprávneho 
...
Druhá dimenzia poskytla študentom priestor na spätnú väzbu ohľadom testovanej aplikácie (podrobnejšie v tabuľke č. \ref{tab:dotaznik-popis} a prílohe č. \todo{priloha}). Položky z druhej dimenzie boli administrované len 
experimentálnej skupine.

\begin{table}[]
  \caption{Charakteristika položiek dotazníka}
  \label{tab:dotaznik-popis}
  \resizebox{\textwidth}{!}{%
  %\begin{tabular}{@{}lll@{}}
    \begin{tabular}{@{}clp{0.7\textwidth}@{}} 
  \toprule
  Číslo & Položka                                                  & Opis                                                                                                                                                                                                       \\ \midrule
  \noalign{\bigskip}1     & Zvoliť obrázok, ktorý najlepšie reprezentuje neurón      & Kľúčom je reprezentácia, ktorá rešpektuje trojdimenzionalitu neurónu, ako aj rozmiestnenie dendritov; distraktormi sú reprezentácie neurónu, ktoré niektorú z uvedených charakteristík nezobrazujú správne \\
  \noalign{\bigskip}2     & Identifikovať, čo je podstatou prenosu nervového vzruchu & Kľúčmi sú správne charakteristiky saltatórnej kondukcie, teda prestup iónov cez membránu a zmena elektrochemického potenciálu; distraktormi sú často identifikované miskoncepcie                           \\
  \noalign{\bigskip}3     & Identifikovať smer, ktorým sa prenáša nervový vzruch     & Kľúčom je obrázok so správne vyznačeným smerom prenosu nervového vzruchu; distraktorom je opak.                                                                                                            \\
  \noalign{\bigskip}4     & Vyjadriť postoj k aplikácii                              & Študenti vyberajú, či by táto aplikácia zatraktívnila vyučovací proces                                                                                                                                     \\
  \noalign{\bigskip}5     & Poskytnúť spätnú väzbu                                   & Otvorená položka s priestorom na poskytnutie dlhšej spätnej väzby                                                                                                                                          \\ \noalign{\bigskip}\bottomrule
  \end{tabular}%
  }
\end{table}
...
\begin{table}[]
  \caption{Súborné štatistické spracovanie (deskriptívna a induktívna štatistická analýza) kvantitatívneho pedagogického výskumu}
  \label{tab:vysledky}
  \resizebox{\textwidth}{!}{%
  ...
  Skupina             & Kontrola           & Experiment           & Kontrola            & Experiment            & Kontrola           & Experiment          & Kontrola             & Experiment             \\ \midrule
  Aritmetický priemer & 0,67               & 0,86                 & 2,67                & 3,00                  & 0,67               & 1,00                & 4,00                 & 4,86                   \\
  Štandardná odchýlka & 0,50               & 0,38                 & 1,41                & 1,29                  & 0,50               & 0,00                & 1,66                 & 1,46                   \\
  Medián              & 1,00               & 1,00                 & 2,00                & 2,00                  & 1,00               & 1,00                & 4,00                 & 4,00                   \\
  Rozptyl             & 0,25               & 0,14                 & 2,00                & 1,67                  & 0,25               & 0,00                & 2,75                 & 2,14                   \\
  Normálne rozdelenie & Nie                & Nie                  & Áno                 & Nie                   & Nie                & Nie                 & Áno                  & Áno                    \\
  Test                & \multicolumn{2}{c}{Mann-Whitney}          & \multicolumn{2}{c}{Mann-Whitney}            & \multicolumn{2}{c}{Mann-Whitney}         & \multicolumn{2}{c}{Independent sample \emph{t}-test} \\
...
\subsubsection{Kvalitatívny výskum}
